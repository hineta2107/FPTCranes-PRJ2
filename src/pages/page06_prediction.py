...
def render(st, root, role="admin"):
    ...
    if examples.empty:
        ...
        with st.container(horizontal=True):
            for index, example in examples.sample(n=min(3, len(examples))).iterrows():
                label = _tr(
    ...
    if st.button(
        ...
        try:
            validate_scenarios(
                [{**candidate, "scenario_id": "preview"}], 
                policy,
                allow_exceptions=True,
                acknowledgements={"preview": policy["policy_hash"]}
            )
        except ScenarioValidationError as exc:
            st.error(_display(str(exc)))
        else:
    ...
    if run:
        try:
            acks = {
                item["scenario_id"]: policy["policy_hash"]
                for item in queue
            }
    ...
    if snapshot:
        ...
        if growth_rows:
            ...
            unique_titles = sorted(growth["job_title"].unique())
            selected_titles = st.multiselect(
                "Select scenarios / Job Title to display:",
            ...
            st.info("💡 **Conclusion:** Prediction of why AI salaries tend to decrease as years of experience increase => Senior engineers may not update their AI skills as quickly as younger generations (due to the rapid pace of technological innovation), leading to a decline in their competitive edge regarding the latest technologies.")
