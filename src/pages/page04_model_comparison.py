...
def render(st, root, role="admin"):
    ...
    with overall:
        ...
        st.markdown(
            """
### 🗓️ Sliding-window Temporal Fold Structure

The development data are divided into six chronological blocks with approximately equal row counts. Each fold uses one block for training and one for validation.

*Sliding-window rule: each fold trains on exactly one block and validates on the next block. The Test set remains isolated throughout model selection.*

| Fold | Block 1 | Block 2 | Block 3 | Block 4 | Block 5 | Block 6 | Test |
|---|---|---|---|---|---|---|---|
