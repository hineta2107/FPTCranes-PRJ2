...
def plot_training_time(comp: pd.DataFrame, out: Path) -> None:
    """Horizontal bar chart — total fit + predict time per model across all folds."""
    if "fit_seconds_mean" not in comp.columns:
        return
    data = comp[["model", "fit_seconds_mean", "predict_seconds_mean"]].copy()
    data = data.sort_values("fit_seconds_mean", ascending=True)

    fig, ax = plt.subplots(figsize=(9.0, 5.2))
    bars = ax.barh(
        data["model"],
        data["fit_seconds_mean"],
        color=PASTEL["blue_dark"],
        alpha=0.88,
        label="Avg fit time / fold (s)",
    )
    ax.barh(
        data["model"],
        data["predict_seconds_mean"],
        left=data["fit_seconds_mean"],
        color=PASTEL["orange"],
        alpha=0.75,
        label="Avg predict time / fold (s)",
    )
    # Annotate total time on each bar
    for bar, row in zip(bars, data.itertuples()):
        total = row.fit_seconds_mean + row.predict_seconds_mean
        ax.text(
            total + 0.002,
    ...
    ax.set_xlabel("Time (seconds) — average per fold")
    ax.set_title("Model Runtime Performance — Fit & Predict Time", color=PASTEL["ink"], fontweight="bold")
    ax.legend(frameon=False, fontsize=9)
    ax.grid(axis="x", alpha=0.15)
    _save(fig, out / "model_comparison_training_time.png")
