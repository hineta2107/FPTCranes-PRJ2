...
def target_profile(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    columns = [c for c in ["job_id", "job_title", "job_category", "experience_level", "years_of_experience", TARGET, "posting_year", "posting_month"] if c in df.columns]
            ...
            rows.append(
                {
                    ...
                    "status": "INVALID_HEADER_TOKEN" if column == "job_category" and value == "AI Engineering" else "OBSERVED",
                }
            )
    return pd.DataFrame(rows)
...
def run_pipeline(data_path: Path | None = None, output_root: Path | None = None) -> PipelinePaths:
    ...
    required = [TARGET, "job_id", "posting_year", "posting_month", "required_skills"]
    missing_required = [c for c in required if c not in raw.columns]
    # Accept either "job_category" (correct) or "AI Engineering" (corrupted header)
    if "job_category" not in raw.columns and "AI Engineering" not in raw.columns:
        missing_required.append("job_category")
    if missing_required:
        ...
        [
            {
                ...
                "invalid_job_category_token_rows": int(raw["job_category"].astype(str).str.strip().eq("AI Engineering").sum()) if "job_category" in raw.columns else 0,
            }
        ]
    )
    ...
    if "AI Engineering" in raw.columns and "job_category" not in raw.columns:
        ...
        corruption_mask = pd.Series(False, index=raw.index)
        corrupted = raw.loc[corruption_mask].copy()
        save_csv(corrupted, paths.basic / "04_corrupted_rows_removed.csv")
        clean = raw.copy()
    ...
    # Runtime performance CSV (separate — per README_RUNTIME_PERFORMANCE.md)
    runtime_cols = [c for c in ["model", "fit_seconds_mean", "predict_seconds_mean"] if c in comparison.columns]
    if runtime_cols:
        save_csv(comparison[runtime_cols].copy(), paths.comparison / "09_model_runtime_performance.csv")
    plot_model_comparison(comparison, paths.comparison)
    plot_training_time(comparison, paths.comparison)
