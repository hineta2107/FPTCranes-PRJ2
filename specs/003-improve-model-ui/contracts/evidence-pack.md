...
| `limitations` | Test exposure, inherited tuning/selection policy, synthetic-looking data and empirical-band limitations |
...
| Candidate test diagnostics | Frozen candidate configs/fitted DEV dependency, Test snapshot, metric code | Recompute required predictions/metrics; reuse immutable matching results if complete |
...
| Benchmarks | Policy, Test row identity, variant prediction identity, selection rule | Reselect deterministically and attach predictions; no extra fitting |
...
`variant_metrics.csv`: shared identities and `evaluation=dev_cv_mean|historical_test`, dataset/fold identity, row count, MAE/RMSE/R²/MedAE. Test entries additionally include `q90_abs_error_usd`, `q90_basis=historical_test_absolute_errors`, `q90_quantile=0.9`, `q90_method=linear`, observed `coverage` and actual denominator. Top-2 and full must share test identities for side-by-side comparison.
...
`variant_encoded_importance.csv`: model/configuration, encoded feature, raw family, importance. `variant_permutation_importance.csv`: model/configuration, raw feature, `mae_increase_usd`, `std_usd`, repeats=12, seed and Test identity. Negative importances are valid evidence; don't clip them.
