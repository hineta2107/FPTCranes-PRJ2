...
- `candidate_test_metrics.csv`: retrospective diagnostics on the already exposed Test set; never used to rerank the family winner;
...
Full and Top-2 use the same Test row identities. Top-2 reuses the full model's frozen estimator settings and was not independently tuned. Current supplemental results are read from the pack rather than copied into source code.

Permutation and encoded importance measure fitted-model reliance, not causal wage effects, fairness or production readiness. Empirical q90 is computed from absolute error on the same Test population; displayed coverage is descriptive, not an independently calibrated guarantee.
